...
function generateTimelineData(): TimelineDataPoint[] {
  ...
  for (let i = 0; i <= 96; i++) {
    ...
    const timestamp = new Date(
      now.getTime() - (24 * 60 - minutesFromStart) * 60 * 1000,
    );
    ...
    for (const supp of DEMO_SUPPLEMENTS) {
      ...
      const ingestionTime = new Date(
        now.getTime() - supp.hoursAgo * 60 * 60 * 1000,
      );
      const minutesSinceIngestion =
        (timestamp.getTime() - ingestionTime.getTime()) / (1000 * 60);
    ...
    <div className="h-full p-4 lg:p-6">
      ...
      <div className="mt-4 border-t border-white/10 pt-3">
        <p className="font-mono text-[10px] text-white/30">
          Decay curves: C(t) = C<sub>max</sub> · e<sup>-kt</sup> where k =
          ln(2)/t<sub>½</sub>
        </p>
      </div>
    </div>
