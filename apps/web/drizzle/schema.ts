...
export const interaction = pgTable("interaction", {
	...
	index("interaction_source_idx").using("btree", table.sourceId.asc().nullsLast().op("uuid_ops")),
	index("interaction_target_idx").using("btree", table.targetId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
...
export const stackItem = pgTable("stack_item", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	...
	index("stack_item_stack_idx").using("btree", table.stackId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
...
export const log = pgTable("log", {
	...
	index("log_logged_at_idx").using("btree", table.loggedAt.asc().nullsLast().op("timestamptz_ops")),
	index("log_user_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const ratioRule = pgTable("ratio_rule", {
	...
	check("ratio_rule_source_supplement_id_not_null", sql`NOT NULL source_supplement_id`),
	check("ratio_rule_target_supplement_id_not_null", sql`NOT NULL target_supplement_id`),
...
export const timingRule = pgTable("timing_rule", {
	...
	check("timing_rule_source_supplement_id_not_null", sql`NOT NULL source_supplement_id`),
	check("timing_rule_target_supplement_id_not_null", sql`NOT NULL target_supplement_id`),
	check("timing_rule_min_hours_apart_not_null", sql`NOT NULL min_hours_apart`),
